...
#[test]
fn context_cli_target_backend_outranks_the_inherited_environment() {
    let ctx = with_cli_target_backend(test_context(OxideConfig::default()), Some("cuda"));
    assert_eq!(configured_target_backend(&ctx, None).unwrap(), "cuda");

    // Without a CLI choice the inherited value wins; an inherited value beats
    // project config, and absence falls back to CUDA.
    let inherited = test_context(OxideConfig {
        env: vec![(TARGET_BACKEND_ENV.to_string(), "maca".to_string())],
        ..OxideConfig::default()
    });
    assert_eq!(configured_target_backend(&inherited, None).unwrap(), "maca");
    ...
    assert_eq!(
        configured_target_backend(&configured, None).unwrap(),
        "maca"
    );
}

#[test]
fn target_backend_device_cfgs_carry_the_maca_wave_size_cfg() {
    assert!(target_backend_device_cfgs("cuda").is_empty());
    assert_eq!(
        target_backend_device_cfgs("maca"),
