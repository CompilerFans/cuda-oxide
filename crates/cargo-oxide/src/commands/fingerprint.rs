...
pub(super) fn passthrough_codegen_fingerprint_with_env(
    ...
    // codegen also change Cargo's rustflags fingerprint. The MACA SDK root
    // participates when the MACA backend is selected: `mxcc` compiles device
    // code from it, so a toolchain change under the same path must re-key.
    ...
    for (key, value) in inherited_env
        .iter()
        .filter(|(key, _)| affects_codegen(key) && key.as_str() != CODEGEN_FINGERPRINT_ENV)
    {
        effective_env.insert(key.clone(), value.clone());
    }
    ...
    effective_env.insert(
        TARGET_BACKEND_ENV.to_string(),
        target_backend.as_bytes().to_vec(),
    );
    if target_backend == "maca" {
        // Re-key device code when the MXMACA compiler binary changes under an
        // unchanged SDK root; an unreadable tool degrades to a marker rather
        // than poisoning every MACA build.
        let mxcc = effective_env
            .get("CUDA_OXIDE_MXCC")
            .map(|bytes| PathBuf::from(String::from_utf8_lossy(bytes).into_owned()))
            .or_else(|| {
                effective_env.get("MACA_PATH").map(|bytes| {
                    PathBuf::from(String::from_utf8_lossy(bytes).into_owned())
                        .join("mxgpu_llvm/bin/mxcc")
                })
            });
        if let Some(mxcc) = mxcc {
            effective_env.insert(
                "__CUDA_OXIDE_MXCC_ARTIFACT".to_string(),
                backend_artifact_digest(&mxcc)
...
pub(super) fn standard_codegen_fingerprint(
    ...
    let base = passthrough_codegen_fingerprint(
        ctx,
        &opts,
        None,
        target_arch,
        &configured_target_backend_or_exit(ctx, None),
        materialization,
    );
    let mut hash = sha2::Sha256::new();
    for bytes in [
