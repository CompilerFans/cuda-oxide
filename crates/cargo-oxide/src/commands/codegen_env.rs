...
/// The child-process variable that selects the device-compilation backend.
///
/// The backend (`BackendOptions::from_env` in `cuda-oxide-codegen`) reads this
/// to switch between the CUDA PTX pipeline and the MetaX MXMACA pipeline, and
/// device-owning procedural macros fold it into the codegen fingerprint.
pub(super) const TARGET_BACKEND_ENV: &str = "CUDA_OXIDE_TARGET_BACKEND";

/// Device cfgs every build must carry for the selected target backend.
///
/// Device crates (cuda-device) gate hardware-dependent constants such as
/// WAVE_SIZE on `cuda_oxide_target_maca`; it must appear on every MACA device
/// build. Merged into the global rustflags by [`apply_codegen_configuration`],
/// so run, build, passthrough, pipeline, debug, and sanitize all pick it up
/// from one place.
pub(super) fn target_backend_device_cfgs(target_backend: &str) -> Vec<String> {
...
/// Resolve the target backend from CLI > inherited environment > project
/// config, defaulting to CUDA. The value is normalized so `MACA` and
/// ` maca ` select the same backend as `maca`.
pub(super) fn resolve_target_backend(
    explicit: Option<&str>,
...
/// The effective target backend for `ctx`, with an explicit CLI choice
/// outranking the inherited environment and project configuration.
pub(super) fn configured_target_backend(
    ctx: &Context,
    explicit: Option<&str>,
) -> Result<String, String> {
    let inherited = std::env::var(TARGET_BACKEND_ENV).ok();
    resolve_target_backend(
        explicit.or(ctx.cli_target_backend.as_deref()),
        inherited.as_deref(),
        project_config_env(ctx, TARGET_BACKEND_ENV),
    )
}

/// `configured_target_backend` for entry points that cannot return an error.
pub(super) fn configured_target_backend_or_exit(ctx: &Context, explicit: Option<&str>) -> String {
...
/// Attach the CLI `--target cuda|maca` choice to the context.
///
/// `None` (the flag omitted) leaves the context unchanged so backend
/// resolution falls back to the environment and project config.
pub fn with_cli_target_backend(mut ctx: Context, explicit: Option<&str>) -> Context {
    ctx.cli_target_backend = explicit.map(str::to_string);
    ctx
}

/// Export the resolved backend onto the child cargo command.
///
/// The child would inherit the parent's value anyway; setting it explicitly
/// publishes the *canonical* spelling, so an inherited `MACA` (accepted for
/// convenience) still reaches the backend as `maca`.
pub(super) fn apply_target_backend(cmd: &mut Command, target_backend: &str) {
...
pub(super) fn apply_codegen_configuration(
    ...
    // The backend selection is exported here and merged into the global cfgs
    // (including the `cuda_oxide_target_maca` device cfg), so every command
    // that applies codegen configuration -- run, build, passthrough, pipeline,
    // debug, and sanitize -- carries it without per-path plumbing.
    ...
    global_cfgs.extend(target_backend_device_cfgs(&target_backend));
    ...
    apply_target_backend(cmd, &target_backend);
    Ok(())
}
