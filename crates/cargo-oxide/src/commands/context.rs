...
pub struct Context {
    ...
    /// Target backend requested by `--target cuda|maca` on the CLI.
    ///
    /// Upstream command signatures dropped their backend parameter, so the
    /// CLI choice travels on the context instead: [`Context`] reaches every
    /// codegen path (run, build, passthrough, pipeline, debug, sanitize,
    /// interop), and `configured_target_backend` ranks it above the inherited
    /// `CUDA_OXIDE_TARGET_BACKEND` and project config. `None` (flag omitted)
    /// keeps that env/config fallback.
    pub cli_target_backend: Option<String>,
}
