...
fn every_limit_reads_and_the_writable_ones_round_trip() {
    if cuda_bindings::CUDA_DRIVER_BACKEND == cuda_bindings::CudaDriverBackend::MacaCuBridge {
        // MXMACA's cu-bridge maps context limits onto MC limits with
        // different statuses (e.g. INVALID_ARGUMENT instead of
        // UNSUPPORTED_LIMIT); the exact round-trip contract is CUDA-only.
        eprintln!("skipping: context limits are not exposed by the cu-bridge driver");
        return;
    }
