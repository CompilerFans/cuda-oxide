...
impl Verify for VoteSyncAllOp {
    fn verify(&self, ctx: &Context) -> Result<(), Error> {
        ...
        if !(is_integer_width(ctx, op.get_operand(0).get_type(ctx), 32) || is_integer_width(ctx, op.get_operand(0).get_type(ctx), 64))
            || !is_integer_width(ctx, op.get_operand(1).get_type(ctx), 1)
            || !is_integer_width(ctx, op.get_result(0).get_type(ctx), 1)
        {
            return verify_err!(
                op.loc(),
                "nvvm.vote_sync_all requires i32/i64 member mask, i1 predicate, and i1 result"
            );
        }
        Ok(())
    }
...
impl Verify for VoteSyncAnyOp {
    fn verify(&self, ctx: &Context) -> Result<(), Error> {
            ...
            || !is_integer_width(ctx, op.get_result(0).get_type(ctx), 1)
        {
            return verify_err!(
                op.loc(),
                "nvvm.vote_sync_any requires i32/i64 member mask, i1 predicate, and i1 result"
            );
        }
        Ok(())
    }
...
impl Verify for VoteSyncBallotOp {
    fn verify(&self, ctx: &Context) -> Result<(), Error> {
            ...
            || !is_integer_width(ctx, op.get_result(0).get_type(ctx), 64)
        {
            return verify_err!(
                op.loc(),
                "nvvm.vote_sync_ballot requires i32/i64 member mask, i1 predicate, and i64 result"
            );
        }
        Ok(())
    }
