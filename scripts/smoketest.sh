...
maca_remap_category() {
    ...
    if [[ "${TARGET_BACKEND}" == "maca" ]]; then
        case "${cat}" in
            tcgen05|wgmma|ltoir|auto-nvvm|ptx-inspect|nv-feature|hmm|blackwell-mma|ltoir-modern|iket) echo "maca-skip" ;;
            *) echo "${cat}" ;;
        esac
    else
        echo "${cat}"
